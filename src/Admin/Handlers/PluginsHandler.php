...
class PluginsHandler
{
    ...
    public function runAction()
    {
        ...
        if (false === wp_verify_nonce($nonce, 'premmerce_plugins_actions')) {
            echo json_encode(
                array(
                    'success'      => false,
                    'errorMessage' => sprintf(
                        __('Sorry, You are not allowed to %s this plugin.', 'premmerce'),
                        $option && in_array(strtolower($option), array( 'activate', 'install' )) ? $option : 'operate'
                    ),
                )
            );
            wp_die();
